...
test('computes footer navigation from the server-side page URL', () => {
	...
	const pages = [
		{ type: 'page', name: '中国软件杯', url: '/jsyzy/js/zgrjb/' },
		{
			type: 'page',
			name: '服务外包创新创业大赛',
			url: '/jsyzy/js/fwwbcxcyds/',
		},
		{
			type: 'page',
			name: '计算机程序设计大赛',
			url: '/jsyzy/js/jsjcxsjds/',
		},
	];
	const tree = { type: 'root', name: 'Docs', children: pages };
	const items = navigationModule.getPageFooterItems(tree, pages[1]);
	assert.equal(items.previous?.url, '/jsyzy/js/zgrjb/');
	assert.equal(items.next?.url, '/jsyzy/js/jsjcxsjds/');
});
